...
function EnterWebsiteBtn()
	uiapi:HyperLink("https://godswarorigin.net/");
end
...
function ToptenBtn_OnClick()
	...
	--local camp = PlayerAPI:GetGameData():GetFaction()
		local str  = "https://godswarorigin.net/rankings"
	uiapi:HyperLink(str);
...
function SeverTime_OnHov()
	...
	str=BC_X0_1;
	win = uiapi:GetElement("TargetInfo");	
...
local chatElements = {
    ...
    "MessageWin1",
    "RecentMessageWin",
    "RecentMessageListBox",
    "RecentMessageUpBtn",
    "RecentMessageDownBtn",
    "HistoryMessageScroll",
    "BtnWin",
}

function ChatUI_Toggle()
    isChatVisible = not isChatVisible

    for _, elementName in ipairs(chatElements) do
        local win = uiapi:GetElement(elementName)
        if win then
            win:Visible(isChatVisible)
        else
            print("[ChatUI_Toggle] Element not found:", elementName)
        end
    end

	local hideButton = uiapi:GetElement("HideChat")
    if hideButton then
        hideButton:Visible(true)
        hideButton:Enable(true)
