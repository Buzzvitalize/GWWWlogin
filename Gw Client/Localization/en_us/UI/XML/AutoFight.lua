...
function AutoFight_OnLoad()
	...
	tabBar = this:GetChild("AutoTabBar");
	skill_Tab = tabBar:GetChild("AutoTab");

	skill_checkbox = skill_Tab:GetChild("checkboxuseskill");
	attack_checkbox = skill_Tab:GetChild("checkboxautofight");
	HP_checkbox = skill_Tab:GetChild("checkboxhp");
	MP_checkbox = skill_Tab:GetChild("checkboxmp");
	HP_edit = skill_Tab:GetChild("edithp");
	MP_edit = skill_Tab:GetChild("editmp");
	--作废
	--red_checkbox = skill_Tab:GetChild("checkboxred");
	--blue_checkbox = skill_Tab:GetChild("checkboxblue");
end
...
function AutoFight_OnUpdate()
	if AutoFightAPI:IsRunAutoFight() then
		...
		if hp > 0 and dead then
			ReviveBGWin:Visible(false);
			dead = false;
		end
...
function AutoRun_OnClick(isRobot)
	if isRobot then
		if first_open then
		AutoInit();
		end
		...
		if not autoskill and not autoattack then
			win:Visible(true);
		else
			if not autoattack and not AutoFightAPI:HasSkillOnUI() then
				win:Visible(true);
			end
		end
	end

	AutoFightAPI:AutoRunSwitch(isRobot);
end
